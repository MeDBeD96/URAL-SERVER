...
Система усталости: 	1. Каждую секунду отнимается 1 ед
					2. При выполнении какого то действия отнимается много
					3. Усталость снимать дома или выпив кофе 
					4. Отнимается в зависимости от голода, если давно не ел, то быстро
...
Пункты APED:
				1.Состояние здоровья (сделать с новым видом диалога) 
				2. Действия (Позвонить, отправить SMS) 
				3. Меню организации 
				4. Настройки (показывать карту, часы) 
				5. Помощь
*/
...
new Float: CheckpointsLetters[MAX_CHECKPOINTS][3] =
{
        {-1966.5238,-2463.4216,30.6250},
        {-1987.0210,-2438.1313,30.6250},
        {-1998.2794,-2424.0972,30.6250},
        {-2012.2651,-2406.2288,30.6250},
        {-2023.0309,-2396.5066,30.6250},
        {-2050.0730,-2379.8232,30.6318},
        {-2059.6628,-2372.6951,30.6250},
        {-2061.0964,-2360.6780,30.6318},
        {-2058.2920,-2347.9277,30.6250},
        {-2046.5280,-2337.4832,30.6250},
        {-2037.9733,-2335.7849,30.6250},
        {-2027.1008,-2341.9080,30.6250},
        {-2031.3462,-2351.5891,30.6250},
        {-2022.9517,-2373.7461,30.6250},
        {-2007.9552,-2385.8694,30.6250},
        {-1986.8928,-2394.5244,30.6250},
        {-1977.8217,-2400.8423,30.6250},
        {-1972.1143,-2411.7004,30.6250},
        {-1961.2272,-2414.4856,30.6250},
        {-1952.0211,-2428.3303,30.6250},
        {-1942.1569,-2434.4304,30.6250},
        {-1931.6995,-2463.5627,30.9368},
        {-1945.3590,-2473.4722,31.2031}
};
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	...
	{
	    if(GetPVarInt(playerid, "letters_work"))
		{
			TogglePlayerControllable(playerid, 0);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 1, 0, 1);
			SetTimerEx("LettersT",4000,false,"i",playerid);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if (newkeys == KEY_SUBMISSION && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && !NoEngine(vid))
    {
        ...
        if (GetPVarInt(playerid, "RentCar") == vid)
    ...
    if (newkeys == 4 && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && !NoEngine(vid))
    {
  		GetVehicleParamsEx(vid,engine,lights,alarm,doors,bonnet,boot,objective);
  		if(!lights)	SetVehicleParamsEx(vid,engine,true,alarm,doors,bonnet,boot,objective);
	...
	if (newkeys == 131072)
		{		
			...
			|| PlayerInfo[playerid][pMember] == 2 && IsPlayerInRangeOfPoint(playerid, 15.0, -2089.7480,-2322.1328,30.6250) && IsPlayerInAnyVehicle(playerid))
			{
				if(LSPDGateOpen) 
				{
			...
			if(IsPlayerInRangeOfPoint(playerid, 10.0, -2150.0566,-2395.3901,30.6250) && PlayerInfo[playerid][pMember] == 1)
			{
				if(BankGateOpen) 
				{
			...
			if(IsPlayerInRangeOfPoint(playerid, 10.0, -2045.9153,-2506.9226,31.2927) && PlayerInfo[playerid][pMember] == 1)
			{
				if(CongressGateOpen) 
				{
			...
			if(IsPlayerInRangeOfPoint(playerid, 10.0, -2397.3569,-2194.6267,33.3970) && PlayerInfo[playerid][pMember] == 5)
			{
				if(FixGateOpen) 
				{
			...
			if(IsPlayerInRangeOfPoint(playerid, 10.0, -2388.1277,-2181.3235,33.4875) && PlayerInfo[playerid][pMember] == 5)
			{
				if(FixGate2Open) 
				{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
	{
		...
		case DIALOG_ID_SETLEADER:
        {
            ...
            PlayerInfo[target][pMember] = listitem + 1;
            PlayerInfo[target][pLeader] = listitem + 1;
					...
					case 0:
					{
							...
							foreach(Player, i)
							{
								...
								if(meme == PlayerInfo[i][pMember])
								{
									...
									ShowPlayerDialog(playerid, DIALOG_ID_BACKTOLMENU, D_S_M, ""COL_ORANGE"Члены организации онлайн", buffer, "Назад", "Закрыть");
								}
							}
					}
					case 1:
		...
		case DIALOG_ID_INVITEMEMBER:
	    {
	        if(response)// Если игрок нажал первую кнопку
	        {
				...
				if (PlayerInfo[playerid][pLeader] == 1) { leader = "Конгресс округа 'Angel Pine'"; }
			 	if (PlayerInfo[playerid][pLeader] == 2) { leader = "Полиция округа 'Angel Pine'"; }
			 					
				format(string,sizeof(string), ""COL_BLUE"\n[%d] %s хочет принять Вас во организацию "COL_WHITE"%s\n",playerid,Name(playerid),fraction_name[PlayerInfo[playerid][pLeader]]);
				ShowPlayerDialog(strval(inputtext), DIALOG_ID_INVITEMEMBERYESNO, D_S_M,""COL_ORANGE"Приглашение",string, "Принять", "Отмена");
	        }
	        else// Если игрок нажал Esc, вернём ему диалог
	        {
	            ShowPlayerLeaderDialog(playerid);
	    ...
	    case DIALOG_ID_LAYOFFMEMBER:
	    {
	        if(response)// Если игрок нажал первую кнопку
	        {
				...
				format(string,sizeof(string), ""COL_BLUE"\n[%d] %s уволил вас из организации "COL_WHITE"%s\n",playerid,Name(playerid),leader);
				ShowPlayerDialog(strval(inputtext), DIALOG_ID_NONE, D_S_M,""COL_ORANGE"Увольнение",string, "Принять", "");
		...
		case DIALOG_ID_RENTMOPEDON:
	    {
	        ...
	        else	SetPVarInt(playerid, "RentCar", 0);
		}
...
/////////////////////////////////////////////                    Команды Команды               / ///////////////////////////
CMD:letters(playerid, params[])
{
	StartLettersWork(playerid);
	return 1;
}

CMD:stopletters(playerid, params[])
{
	StopLettersWork(playerid);
	return 1;
}
...
CMD:unleader(playerid, params[])
{
	...
	if(sscanf(params,"is[30]",params[0],params[1])) return SendClientMessage(playerid,COLOR_GREY,"CMD: /unleader [ID игрока] [Причина]");
	
	if (params[0] == INVALID_PLAYER_ID || !IsPlayerConnected(params[0]) || gPlayerLogged[params[0]] == false) return SendClientMessage(playerid, COLOR_GREY,
	"Этот игрок сейчас не в сети");
	
	if(PlayerInfo[params[0]][pLeader] == 0) return SendClientMessage(playerid,COLOR_GREY,"Игрок не является лидером организации.");
	
	new string[145];
	
	format(string,sizeof(string), ""COL_BLUE"\nАдминистратор [%d] %s снял вас с лидерства организации "COL_WHITE"%s\n",playerid,Name(playerid),
	fraction_name[PlayerInfo[params[0]][pMember]]);
	ShowPlayerDialog(params[0], DIALOG_ID_NONE, D_S_M,""COL_ORANGE"Информация",string, "Принять", "");
	
	PlayerInfo[params[0]][pMember] = 0;
	PlayerInfo[params[0]][pLeader] = 0;
	PlayerInfo[params[0]][pMemberWarn] = 0;
	PlayerInfo[params[0]][pMemberSkin] = 0;
	
	format(string,sizeof(string), "Администратор [%d] %s снял лидерство организации %s c игрока [%d] %s, причина: %s",
	playerid,Name(playerid),fraction_name[PlayerInfo[params[0]][pMember]],params[0],Name(params[0]), params[1]);
	WriteLog("Leaders",string);
	return 1;
}
...
StartLettersWork(playerid) // Начинает работу.
{
	if(GetPVarInt(playerid, "IsWork") != 0) return SendClientMessage(playerid, COLOR_GREY,"Закончите предыдущую работу.");
    if(GetPVarInt(playerid, "letters_work")) return 1;
    SetPVarInt(playerid, "letters_work", 1);
	SetPVarInt(playerid, "IsWork", 1);
    SetPlayerAttachedObject(playerid,3,1575,5,0.124000,0.024999,-0.003000,-66.900001,1.800001,5.799997,0.541999,0.690999,0.415999);
	SetPlayerRaceCheckpoint(playerid,0,CheckpointsLetters[0][0],CheckpointsLetters[0][1],CheckpointsLetters[0][2],CheckpointsLetters[0][0],CheckpointsLetters[0][1],
	CheckpointsLetters[0][2],0.5);
	return 1;
}

StopLettersWork(playerid, bool:success = false) // Заканчивает работу.
{
        if(!GetPVarInt(playerid, "letters_work")) return;
        DeletePVar(playerid, "letters_work");
        DeletePVar(playerid, "letters_check");
		DeletePVar(playerid, "IsWork");
        if(success)
        {
            PlayerInfo[playerid][pMoney] += 250; // Заработная плата.
            SendClientMessage(playerid, COLOR_WHITE, "Вы заработали 250$"); // Выводит текст
            RemovePlayerAttachedObject(playerid,3);

        }
        DisablePlayerRaceCheckpoint(playerid); // Удаляет чекпоинт.
}

NextLettersCheck(playerid) // Показывает следующий чекпоинт.
{
        if(!GetPVarInt(playerid, "letters_work")) return;
        new check = GetPVarInt(playerid, "letters_check");
        check++;
        if(check >= sizeof CheckpointsLetters) StopLettersWork(playerid, true); // Если вы на последней точке, заканчивает работу.
        else
		{
				SetPlayerRaceCheckpoint(playerid,0,CheckpointsLetters[check][0],CheckpointsLetters[check][1],CheckpointsLetters[check][2],CheckpointsLetters[check][0],
				CheckpointsLetters[check][1],CheckpointsLetters[check][2],0.5);
                SetPVarInt(playerid, "letters_check", check);
        }
}
...
ShowPlayerLeaderDialog(playerid)
{
	ShowPlayerDialog(playerid, DIALOG_ID_LMENU, D_S_L, ""COL_ORANGE"Меню лидера", ""COL_WHITE"\
	[1] Члены организации\n\
	[2] Принять игрока\n\
	[3] Уволить игрока\n\
	[4] Дать выговор\n\
...
public PlayerUpdate(playerid)
{
	...
	//new check [4];		format(check,sizeof(check),"%d", GetAroundPlayerVehicleID(playerid, 3.0)); 		GameTextForPlayer(playerid, check, 1000, 3);	
    ...
    {	
		...
		if(VID > 0)
		{
			...
			if(VHP < 300.0 && GetPVarInt(playerid, "RentCar") != 0)
			{
...
forward LettersT(playerid);
public LettersT(playerid)
{
 	TogglePlayerControllable(playerid, 1);
 	NextLettersCheck(playerid);
	return true;
}
